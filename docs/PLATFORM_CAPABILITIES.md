...
> **版本**: 2.2  
> **更新日期**: 2026-09-04  
> **定位**: 面向中小企业和个人开发者的 AI Agent 平台  
> **核心特点**: 渐进式弹性架构 + AgentScope 满血版 + 动态工具系统 + 流水线编排 + 安全护栏
...
Claw Agent 不是企业级 SaaS 平台,而是**面向中小企业和个人开发者的私有化 AI Agent 运行环境**:

1. **渐进式弹性架构**: 单机模式零依赖启动（Redis 降级为内存）,多实例模式 Redis 共享状态支持水平扩展
...
| **中小企业(10-200人)** | 内部文档检索、客服机器人、数据分析、流水线自动化 | 多租户隔离、权限控制、多实例扩展 |
| **技术爱好者** | 探索 Agent 能力、测试新工具/MCP | 开箱即用、扩展性强 |

### ❌ 不是什么

- ❌ **不是大型 SaaS**: 不支持超大规模并发(万级用户)、没有 SLA 保证
...
- ✅ 数据库表 `sys_dept` 已创建
- ✅ `DeptController` 存在
- ✅ **前端页面已实现**: `/system/dept` 部门管理页
- ✅ 部门树形结构展示、新增/编辑/删除
- ⚠️ **业务逻辑简化**: 用户表中 `dept_id` 字段未强制关联
...
- ✅ **前端页面已实现**: `/system/online` 在线监控页
- ⚠️ **功能简化**: 仅记录在线用户到内存 Map (`OnlineUserTracker`)
- ⚠️ **无实时推送**: 无 WebSocket 实时推送，需手动刷新
...
1. **单机部署**: Docker Compose 一键启动 MySQL + Backend + Frontend（Redis 可选）
2. **免费模型**: 配置 Ollama (本地) 或 Groq (云端免费额度)
3. **核心功能**: 聊天对话 + 工具调用 + 笔记管理
4. **可选功能**: Token 统计、MCP 服务器(Git/SQLite)

### 中小企业 (10-200人)

1. **多租户隔离**: 为每个部门/项目创建独立租户
2. **权限控制**: 平台管理员管理租户,租户管理员管理本团队用户
3. **多实例部署**: Nginx 负载均衡 + Redis 共享状态,水平扩展应对高并发
...
3. ~~**Gemini 模型支持**: 接入 Google Gemini 模型提供商~~ ✅ 已完成
