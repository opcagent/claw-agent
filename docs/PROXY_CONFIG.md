...
Claw Agent 的联网搜索功能（`web_search` 工具）使用 DuckDuckGo HTML 端点，该服务在中国大陆网络环境下需要配置 HTTP 代理才能正常访问。
...
1. **仅影响联网搜索**：代理配置只影响 `web_search` 工具，不影响其他功能
2. **动态生效**：修改代理配置后需要重启应用才能生效
3. **安全性**：生产环境建议使用可信的代理服务，避免敏感信息泄露
4. **超时时间**：当前设置为 15 秒，如果代理较慢可以适当增加
